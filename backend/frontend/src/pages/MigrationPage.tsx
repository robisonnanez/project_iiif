...
export function MigrationPage({ config, notify }: { config: AppConfig | null; notify: Notify }) {
  ...
  return <>
    ...
    <Card className="migration-logs"><div className="card-heading"><div><h2>Registros</h2><p>Salida acumulada del proceso.</p></div></div><pre className="log-output">{status?.logs?.length ? status.logs.join("\n") : "Sin registros."}</pre></Card>
    {confirmOpen && <Modal title="Contexto de migración" description="Selecciona el proyecto y el tenant antes de iniciar la migración." onClose={() => !busy && setConfirmOpen(false)}><ScopeFields projects={projects} project={project} tenant={tenant} allowDynamic={Boolean(config?.projects.allow_dynamic_tenants)} onProject={changeProject} onTenant={setTenant} /><FormField label="Ruta de origen">{(id) => <Input id={id} value={sourcePreview} readOnly />}</FormField>{error && <Alert tone="danger">{error}</Alert>}<div className="modal-actions"><Button variant="secondary" disabled={busy} onClick={() => setConfirmOpen(false)}>Cancelar</Button><Button disabled={busy} onClick={start}>{busy ? <Spinner label="Iniciando" /> : "Iniciar migración"}</Button></div></Modal>}
